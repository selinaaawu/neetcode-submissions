...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ## TRIE & HASH SET | time: O(m * n * 4 * 3^(max word length) + sum length all word)
        ...
        def dfs(r, c, node):
            ...
            board[r][c] = char                  # restore cell to original
